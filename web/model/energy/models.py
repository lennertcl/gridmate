...
@dataclass
class Optimization:
    """Domain model for energy optimization configuration"""

    enabled: bool = False
    optimization_frequency: str = 'hourly'  # hourly, daily, realtime
    forecast_horizon: int = 24  # hours
    optimization_goal: str = 'cost_minimization'  # cost_minimization, emission_reduction, balanced
    last_updated: datetime = field(default_factory=datetime.now)
    ...
    def from_dict(cls, data: Dict) -> 'Optimization':
        ...
        return cls(
            ...
            optimization_frequency=data.get('optimization_frequency', 'hourly'),
            forecast_horizon=data.get('forecast_horizon', 24),
            optimization_goal=data.get('optimization_goal', 'cost_minimization'),
            last_updated=last_updated,
        )
